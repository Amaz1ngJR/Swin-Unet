...
class DiceLoss(nn.Module):
    ...
    def _one_hot_encoder(self, input_tensor):
        ...
        output_tensor = torch.cat(tensor_list, dim=1) #[1,2,512,512]
    ...
    def forward(self, inputs, target, weight=None, softmax=False):
        ...
        target = self._one_hot_encoder(target)#[1,512,512]=>
        if weight is None:
...
def test_single_volume(image, label, net, classes, patch_size=[512, 512], test_save_path='/home/yjr/Swin-Unet/output/predictions', case=None, z_spacing=1):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    _, x, y = image.shape
