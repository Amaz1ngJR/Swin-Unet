...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            outputs = model(image_batch) # [1,2,512,512]
            outputs = outputs.squeeze(0)
            #loss_ce = ce_loss(outputs, label_batch[:].long())#output 0?1  label_batch[1,512,512]
            ...
            if(i==1000):
                ...
                cv2.waitKey(0)
                i=0
            #sample = {'image': image, 'label': label_batch}#标签被转换为长整型 
